...
- Shared-plan details, the shared list and main-app RSVP cards refresh every five seconds while visible, and refresh on focus, page restoration and restored connectivity. Skipped ticks during writes or hidden pages keep their next refresh scheduled. Late detail responses cannot overwrite newer revisions. Manual refresh and connection/error states are visible; unavailable memberships clear the displayed plan.
...
2026-09-07 RSVP repair: the first real invitee's Going response was confirmed saved in production. Organizer visibility now includes a live response card in the main app, participant totals in the shared list and a persistent summary above the open plan. Changed responses are announced in-app. The two-account local browser test confirmed automatic updates in the main app and open plan; the suite now has 103 passing tests, with all release builds and dependency checks passing. The repair uses the existing schema and member-scoped reads; no database migration or real-plan mutation is required.
...
- Personal cloud collections refresh on app load. Shared responses refresh automatically throughout the hosted app, including the main app, list and open shared plan. RSVP changes are shown in-app; email, SMS and push notifications are not sent. There is no offline mutation queue or automatic merge. Personal save failures pause editing; shared failures show errors and keep the last confirmed state/drafts for retry. Manual suggestion IDs and plan source keys make ordinary retries idempotent.

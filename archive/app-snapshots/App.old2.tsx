...
const STORAGE_MEMORY = 'thingsNearbyMemoryV27';
const STORAGE_LOCATION = 'thingsNearbyLocationV27';
...
out center tags;`;
    ...
    // overpass-api.de is consistently returning 406 for this Expo/Android request shape,
    // so skip it for now. The 406 itself is fast, but retrying known-bad endpoints adds noise
    // and makes the app feel less reliable. Kumi is the endpoint that has returned real results.
    const endpoints = [
    ...
    addLog('Skipping overpass-api.de because it is returning repeated 406 responses in Expo Go');
  ...
  const findThings = async () => {
    ...
    try {
      ...
      const raw75 = await fetchFoodCards(loc, 7.5);
      ...
      // Only widen if zero or nearly zero, to avoid minutes-long waits.
      if (filtered.length < 3) {
        addLog(`Only ${filtered.length} matches; trying wider 14.9mi search.`);
        const raw149 = await fetchFoodCards(loc, 14.9);
  ...
  const hoursText = (card?: PlaceCard | null) => {
    const hours = card?.tags?.opening_hours;
    return hours ? `Hours: ${hours}` : 'Hours: not available in OpenStreetMap';
  };

  const mapQueryForCard = (card: PlaceCard) => {
    if (card.kind === 'pairing') return pairingSearchQuery(card);
  ...
  const openPlanDirections = () => {
    ...
    const foodQ = selectedFoodCard ? mapQueryForCard(selectedFoodCard) : '';
    const activityQ = selectedActivityCard ? mapQueryForCard(selectedActivityCard) : '';
    const destination = selectedActivityCard ? activityQ : foodQ;
    const waypoints = selectedFoodCard && selectedActivityCard ? `&waypoints=${encodeURIComponent(foodQ)}` : '';
    ...
    addLog(`Open plan directions: origin=${origin}; food=${foodQ || 'none'}; activity=${activityQ || 'none'}`);
    Linking.openURL(url);
  };
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      {(selectedFoodCard || selectedActivityCard) && (
        <View style={styles.planBox}>
          ...
          {selectedFoodCard && <Text style={styles.planMeta}>{hoursText(selectedFoodCard)}</Text>}
          {selectedActivityCard && <Text style={styles.planLine}>Activity: {selectedActivityCard.title}</Text>}
          {selectedActivityCard && <Text style={styles.planMeta}>{hoursText(selectedActivityCard)}</Text>}
          ...
          <View style={styles.actions}>
            <TouchableOpacity style={styles.action} onPress={openPlanDirections}><Text>Directions to plan</Text></TouchableOpacity>
      ...
      {visibleCards.map((card) => (
        <View key={card.id} style={styles.card}>
          ...
          <Text style={styles.meta}>{hoursText(card)}</Text>
...
const styles = StyleSheet.create({
  ...
  planMeta: { color: '#6b7280', fontSize: 14, fontWeight: '700', marginBottom: 4 },
